...
test("renders the complete public report", async () => {
  ...
  assert.match(html, /The three survey questions, answered/);
  assert.match(html, /Overview of the three open-ended questions/);
  assert.match(html, /Question (?:<!-- -->)?01 · Future concerns/);
  assert.match(html, /Question (?:<!-- -->)?02 · Personal sensing/);
  assert.match(html, /Question (?:<!-- -->)?03 · Institutional practices/);
  ...
  assert.ok((html.match(/Answer at a glance/g) ?? []).length >= 3);
  assert.equal((html.match(/class="overview-theme-list"/g) ?? []).length, 3);
  assert.match(html, /Concerns named by respondents/);
  assert.match(html, /Climate, ecology and liveability/);
  assert.match(html, /Funding, legitimacy and institutional pressure/);
  assert.match(html, /Arts education and creative practice is the leading concern/);
  assert.match(html, /Foresight here is social infrastructure as much as an analytical technique/);
  assert.match(html, /Conversation is common; formal foresight is not/);
...
test("renders a clear, expandable and verifiable AREAS analysis", async () => {
  ...
  assert.match(html, /02 · Where power sits/);
  assert.match(html, /The biggest decisions are mostly made elsewhere/);
  ...
  assert.match(html, /06 · What the survey cannot show/);
  assert.match(html, /The most powerful actors are missing from the responses/);
  assert.match(html, /Position view/);
  ...
  assert.match(html, /no global actors or C-level decision-makers answered the survey/i);
  assert.match(html, /The main decision-makers are missing/);
  ...
  assert.match(html, /No second researcher, respondent review or outside evidence was used/);
